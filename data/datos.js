let menuC = [
  { name: "Recomendación", label : "vino_Recomendado" },
...
function crear() {
  ...
  `<link rel="stylesheet" href="./styles/mainCategoria.css"><header><img src="./img/logoGarufa.png"></header><main id="main"></main><div class="side" id="A"></div>`;
  for (let i = 0; i < menuC.length; i++) {
    document.getElementById("main").innerHTML +=
    `<div `+(i==0 ? 'id="recomendado"':"")+`onclick = "categoria(` + i + `)">` + menuC[i].name + `</div>`;
  }
  document.getElementById("body").innerHTML +=
  `<div class="side" id="B"></div>`;
  if (parseInt(a) == 1) {
    document.getElementById("body").innerHTML =
...
function botellas(a, b) {
  document.getElementById("body").innerHTML =
    `<link rel="stylesheet" href="./styles/mainBotellas.css"><a id= "volver" onclick="` +
    (a == 1 ? "categoria(1)" : "crear()") +
    `"><img src="img/volver.png"></a>
  <style> main{grid-template-columns: repeat(` +
    (a == 1 ? botella[a][b].length : botella[a].length) +
    `,` +
    (botella[a].length > 3 ? "35" : 100 / botella[a].length) +
    `%)}</style>
  <header><p><img src="./img/logoGarufa.png"><stronge><img src="./img/flags/0` +
    (a > 8 ? 8 : a) +
    `.png"><br>` +
    (a == 1 ? zac[b].name : menuC[a].name) +
    `</stronge></p></header>
  <main id ="main">`;
